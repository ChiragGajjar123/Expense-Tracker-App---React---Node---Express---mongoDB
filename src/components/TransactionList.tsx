        ...
        {filteredTransactions.length > 0 ? (
          <div className="divide-y divide-gray-100">
            {filteredTransactions.map((t) => {
              ...
              return (
                <div key={t.id} className="p-3 sm:p-4 flex items-center gap-3 sm:gap-4 hover:bg-gray-50 transition-colors group">
                  <div 
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm sm:text-base font-semibold text-gray-900 truncate">{t.title}</h4>
                    <div className="flex items-center gap-1.5 text-[10px] sm:text-sm text-gray-500">
                  ...
                  <div className="text-right flex flex-col items-end gap-1 sm:gap-2">
                    <div className={`text-sm sm:text-base font-bold flex items-center gap-1 ${
                      ...
                      {t.type === 'income' ? <ArrowUpRight className="w-3 h-3 sm:w-4 sm:h-4" /> : <ArrowDownLeft className="w-3 h-3 sm:w-4 sm:h-4" />}
                      ${t.amount.toLocaleString()}
                    </div>
                    
                    <div className="flex items-center gap-1 sm:gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={() => onEdit(t)}
                        className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer active:bg-blue-100"
                      >
